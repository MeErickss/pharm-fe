  ...
  return (
    <div className="w-full h-full bg-gray-100 p-4">
      <div className="w-full border border-gray-300 rounded-lg bg-white shadow">
        <div className="grid grid-cols-[3rem_7rem_7rem_8rem_5rem] bg-gray-200 font-semibold text-gray-700 p-3 border-b">
          <div>ID</div>
          <div>Unidade</div>
          <div>Abreviação</div>
          <div>Tipo Unidade</div>
          <div>Ações</div>
        </div>

        {dados.map((x) => (
          <div key={x.ID} className="grid grid-cols-[3rem_7rem_7rem_8rem_5rem] p-3 border-b items-center">
            <div>{x.ID}</div>
            <div>{x.UNIDADE}</div>
            <div>{x.ABREVIACAO}</div>
            <div>{x.TIPO_UNIDADE}</div>
