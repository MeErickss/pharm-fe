...
export function EditarParametro({ id, dados, closeModal, param }) {
  ...
  // atualiza campos incluindo grandeza → unidade em cascata
  const handleEdit = (value, key) => {
    ...
    setValoresEditados(prev => {
      ...
      if (key === "grandeza" && typeof value === "object") {
        ...
        updated.unidade = ""; // reset da unidade ao mudar grandeza
      }
      else if (key === "unidade" && typeof value === "object") {
        console.log(updated.unidade)
        console.log(value.unidade)
        console.log("updated.unidade")
        // quando o SelectInputUpdate envia objeto, pega a propriedade correta
        updated.unidade = value.unidade;
      }
      else if (key === "status" || key === "funcao") {
        // status e funcao são strings simples vindas da lista
        updated[key] = typeof value === "string" ? value : value.nome || value.descricao || "";
      }
      else {
        // demais campos texto/número
        updated[key] = value;
      }
      ...
      <div className="flex flex-row justify-between mb-2">
        ...
        <button onClick={closeModal}>
          <svg
            ...
            viewBox="0 0 384 512"
            width="16"
            className="fill-red-500 hover:fill-red-900"
          >
        ...
        {Object.keys(itemInicial).map(key => (
          key !== "id" && key !== "unidade" && (
            <div key={key}>
              ...
              {["status", "funcao", "grandeza", "unidade"].includes(key) ? (
                <SelectInputUpdate
                  param={param}
                  table={key}
                ...
                <input
                  ...
                  min={key === "valor" ? valoresEditados.vlMin || 0 : undefined}
                  max={key === "valor" ? valoresEditados.vlMax || undefined : undefined}
